...
function JobForm({ initial = emptyJob, onSave, onCancel, loading }) {
  ...
  const setDate = (key, val) =>
    setForm((f) => ({
      ...f,
      importantDates: { ...f.importantDates, [key]: val },
    }));

  const setFee = (key, val) =>
    setForm((f) => ({
      ...f,
      applicationFee: { ...f.applicationFee, [key]: Number(val) },
    }));
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <Field label="General Fee (₹)">
          <input
            ...
            onChange={(e) => setFee("general", e.target.value)}
            className="field-input"
          />
        </Field>
        <Field label="Reserved Fee (₹)">
          <input
            ...
            onChange={(e) => setFee("reserved", e.target.value)}
            className="field-input"
          />
        </Field>
...
export default function AdminClien() {
  ...
  // ── Reload helper (plain async fn, no useCallback)
  ...
  // ── Resolve collection key for delete
  const resolveCollection = (tabId) => {
    const map = {
      jobs: COLLECTIONS.JOBS,
      results: COLLECTIONS.RESULTS,
      blogs: COLLECTIONS.BLOGS,
      admitCards: COLLECTIONS.ADMIT_CARDS,
    };
  ...
  if (user === undefined) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-gray-400 animate-pulse text-sm font-medium">Loading...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {toast && (
        <div
          className={`fixed top-4 right-4 z-50 px-5 py-3 rounded-xl shadow-lg text-white font-semibold text-sm transition-all ${
                      ...
                      {activeTab === "jobs" && (
                        <>
                          ...
                          <td className="px-5 py-4 font-medium">
                            {item.posts?.toLocaleString("en-IN")}
                          </td>
                          <td className="px-5 py-4">
                            <span
                              className={`badge ${
